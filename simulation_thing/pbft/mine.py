...
class PBFTNode:
    ...
    def send_prepare(self, node_id, block):
    ...
    def broadcast_prepare(self, block):
        ...
        for node_id in range(self.num_nodes):
            if node_id != self.node_id:
                self.send_prepare(node_id, block)#sending prepare msg to all the other nodes, i.e. broadcasting prepare msgs
    ...
    for request_num in range(num_requests):#all the requests are the same here
        ...
        for node in nodes:
            if node != random_node:
                ...
                #recieve prepare for each node, has to be done by each node rather than the send_prepare function
